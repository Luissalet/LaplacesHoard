...
| `data_register` | no | `path`, `name?`, `options?` (`delimiter`, `header`, `sheet`/`sheets`, `tables`, `glob`) | the new dataset's schema and profile; Excel/SQLite: `sheets`/`tables` and a short entry per dataset |
...
| `data_chart` | yes | `sql`, `kind` (`bar line area scatter histogram pie heatmap`), `x`, `y?`, `color?`, `title?` | a JSON text part (`id`, `cite`, `spec_summary`, `row_count`, `png_bytes`) followed by the chart as MCP `ImageContent` (PNG, ≤ ~200 KB) |
